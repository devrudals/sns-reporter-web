...
const PAGE_TITLES: Record<string, string> = {
  ...
  '/final-works': '완성본',
  '/mobile': '모바일 뷰',
  '/admin/users': '회원 명단 관리',
  '/admin/contents': '콘텐츠 현황 관리',
  '/admin/settings': '마감일 설정',
};
      ...
      <ModalProvider>
        ...
        <div className="w-full min-h-screen bg-slate-100 p-0 m-0">
          {/* 화면에는 보이지 않지만 보조기술이 페이지 주제를 알리는 제목 —
              ...
              landmark 밖에 홀로 놓이지 않도록 header로 감싼다. */}
          <header>
            <h1 className="sr-only">모바일 뷰</h1>
          </header>
          {children}
        </div>
      </ModalProvider>
      ...
      <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: 'transparent', position: 'relative' }}>
        ...
        >
          ...
          <nav style={{ padding: '0 0.5rem', display: 'flex', flexDirection: 'column', gap: '0.25rem', flex: 1 }}>
            ...
            <Link href="/resources" className={getLinkClass('/resources')}>
              ...
              자료실
              <span style={{ marginLeft: 'auto', fontSize: '0.62rem', fontWeight: 700, color: 'var(--color-text-muted)', border: '1px solid var(--color-border)', borderRadius: '999px', padding: '1px 6px', whiteSpace: 'nowrap' }}>준비 중</span>
            </Link>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', width: '100%' }}>
          <header className="pc-header-glass px-4 sm:px-6 lg:px-8" style={{ height: '68px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 30 }}>
            ...
            {/* 좁은 화면에서는 이 묶음이 먼저 줄어든다 — 예전에는 검색창이 200px를
                끝까지 붙들어 오른쪽 묶음을 화면 밖으로 밀어냈고, 그 결과 휴대폰에서
                로그아웃 버튼을 누를 수 없었다. */}
            ...
            <form 
              ...
              className="w-[120px] sm:w-[280px] md:w-[340px]"
              style={{ 
                display: 'flex', 
                alignItems: 'center', 
                flexShrink: 1,
            ...
            >
              <input type="text" name="q" aria-label="콘텐츠, 작성자, 키워드 검색" placeholder="콘텐츠, 작성자, 키워드 검색" autoComplete="off" defaultValue={typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('q') || '' : ''} style={{ border: 'none', backgroundColor: 'transparent', outline: 'none', flex: 1, fontSize: '0.88rem', color: 'var(--color-text-main)', minWidth: 0 }} />
              <button type="submit" aria-label="검색" title="검색" style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', display: 'flex', alignItems: 'center', marginLeft: '0.5rem', color: '#94A3B8' }}>
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          <h1 className="sr-only">{PAGE_TITLES[pathname] ?? 'SNS기자단 기획안 관리'}</h1>
