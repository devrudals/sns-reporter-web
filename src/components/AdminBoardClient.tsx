...
const getTeamFilterStyle = (_team: string, isSelected: boolean) => {
  if (!isSelected) {
    return 'bg-transparent text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-white/40 dark:hover:bg-slate-700/40';
  }
    ...
    return (
      ...
      >
        ...
        <div className="text-[11.5px] font-medium flex items-center gap-1.5 min-w-0 bg-slate-900 text-slate-100 px-2.5 py-1.5 rounded-lg border border-slate-800 shadow-2xs">
          ...
          <span className="truncate text-slate-100 font-medium">{triggerInfo.text}</span>
        </div>
      </div>
    );
  };
