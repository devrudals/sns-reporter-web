...
import FinalWorkPreview from '@/components/FinalWorkPreview';
              ...
              <>
                ...
                <div 
                  ...
                  style={{ 
                    ...
                    cursor: 'pointer',
                    position: 'relative',
                    height: `${paneHeight('final')}px`,
                    minHeight: `${paneHeight('final')}px`,
                    maxHeight: `${paneHeight('final')}px`,
                    transition: `height ${PANE_MS}ms ${PANE_EASE}, min-height ${PANE_MS}ms ${PANE_EASE}, max-height ${PANE_MS}ms ${PANE_EASE}`,
                    flexShrink: 0
                  }}
                  className="hover-card"
                >
                  <FinalWorkPreview
                    finalUrl={selectedContent.final_url}
                    title={selectedContent.title}
                    compact={paneCollapsed('final')}
                    meta={
                      <>
                ...
                <div 
                  ...
                  style={{ 
                    ...
                    flexDirection: 'column',
                    gap: '12px',
                    height: `${paneHeight('proposal')}px`,
                    minHeight: `${paneHeight('proposal')}px`,
                    maxHeight: `${paneHeight('proposal')}px`,
                    // 접힌 동안에는 스크롤 막대가 잠깐 나타났다 사라지는 게 보여서 숨긴다.
                    overflowY: paneCollapsed('proposal') ? 'hidden' : 'auto',
                    scrollbarGutter: 'stable',
                    transition: `height ${PANE_MS}ms ${PANE_EASE}, min-height ${PANE_MS}ms ${PANE_EASE}, max-height ${PANE_MS}ms ${PANE_EASE}, padding ${PANE_MS}ms ${PANE_EASE}`,
                    flexShrink: 0
                  }}
                  className="hover-card"
                >
                  ...
                  {/* 다른 카드가 확대돼 이 카드가 접히면 제목 줄만 남긴다(요청 반영). */}
                  {!paneCollapsed('proposal') && (
                    <>
                  <div>
                ...
                <div 
                  ...
                  style={{ 
                    ...
                    flexDirection: 'column',
                    gap: '12px',
                    height: `${paneHeight('feedback')}px`,
                    minHeight: `${paneHeight('feedback')}px`,
                    maxHeight: `${paneHeight('feedback')}px`,
                    overflowY: paneCollapsed('feedback') ? 'hidden' : 'auto',
                    transition: `height ${PANE_MS}ms ${PANE_EASE}, min-height ${PANE_MS}ms ${PANE_EASE}, max-height ${PANE_MS}ms ${PANE_EASE}, padding ${PANE_MS}ms ${PANE_EASE}`,
                    flexShrink: 0
                  }}
                  className="hover-card"
                >
                  ...
                  {/* 다른 카드가 확대되면 '피드백 N' 줄만 남긴다(요청 반영). */}
                  {!paneCollapsed('feedback') && (
